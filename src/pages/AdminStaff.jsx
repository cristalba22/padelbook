    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
      <section className="admin-panel rounded-3xl border border-white/10 p-5 md:p-7">
        ...
        <div className="mt-6 space-y-3">{staff.map((employee) => <div key={employee.id} className="rounded-2xl border border-white/10 bg-black/25 p-4">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => updateEmployee(employee, { active: employee.active === false })} className="btn-outline px-4 py-2 text-xs">{employee.active === false ? "Activar" : "Desactivar"}</button><input type="password" aria-label={`Nueva contraseña para ${employee.name}`} minLength={12} placeholder="Nueva contraseña (12+)" value={resets[employee.id] || ""} onChange={(event) => setResets((current) => ({ ...current, [employee.id]: event.target.value }))} className="field max-w-[220px]" /><button type="button" disabled={Boolean(busy) || (resets[employee.id] || "").length < 12} onClick={() => updateEmployee(employee, { password: resets[employee.id] })} className="btn-outline px-4 py-2 text-xs">Cambiar clave</button></div>
      ...
      <form onSubmit={createEmployee} className="admin-panel h-fit rounded-3xl border border-white/10 p-5 md:p-7">
        ...
        <div className="mt-5 space-y-4"><label className="block text-sm">Nombre<input className="field mt-2" value={form.name} minLength={2} maxLength={100} required disabled={!apiOnline} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} /></label><label className="block text-sm">Email<input className="field mt-2" type="email" value={form.email} required disabled={!apiOnline} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} /></label><label className="block text-sm">Teléfono<input className="field mt-2" value={form.phone} disabled={!apiOnline} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} /></label><label className="block text-sm">Contraseña inicial<input className="field mt-2" type="password" minLength={12} autoComplete="new-password" value={form.password} required disabled={!apiOnline} onChange={(event) => setForm((current) => ({ ...current, password: event.target.value }))} /></label></div>
